...
-- 2. Create the new view with "Best of X" logic
...
WITH RankedResults AS (
    SELECT 
        ...
        r.gender,
        r.birth_year,
        -- Rank races by points (highest first) for each rider per year
        ROW_NUMBER() OVER (
        ...
        CASE 
            ...
            -- Add more years as needed; defaults to 5 if not listed
            ELSE 5 
        END as points_limit
    FROM results res
...
SELECT 
    ...
    -- Total sum of EVERY race
    SUM(points) AS total_points,
    -- Sum of only the top X races based on the yearly mapping
